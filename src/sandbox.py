...
def plot_metrics(algos: dict, metric: str, x_label: str, y_label: str, title: str):
    ...
    bar_positions = [np.arange(goal_count)]
    for _ in range(1, len(algos)):
        bar_positions.append([x + bar_width for x in bar_positions[-1]])

    for map_index in range(map_count):
        fig, ax = plt.subplots()
        
        for i, (algo, data) in enumerate(algos.items()):
            stats = data["stats"][map_index]
            ax.bar(bar_positions[i], stats[metric], label=f"{algo} on {stats['map']}", width=bar_width)
        
        ax.set_xticks([r + bar_width * (len(algos) / 2) for r in range(goal_count)])
        ax.set_xticklabels([str(i + 1) for i in range(goal_count)])
